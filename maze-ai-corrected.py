...
def train_agent():
    ...
    for episode in range(episodes):
        # Track individual episode start time
        ...
        # Calculate and print episode duration
        episode_duration = time.time() - episode_start_time
        print(f"Episode {episode + 1}: {episode_duration:.4f} seconds, Steps: {step_count}")

        # Update tracking variables
        episode_times.append(episode_duration)
        total_time += episode_duration
        highest_time = max(highest_time, episode_duration)
        lowest_time = min(lowest_time, episode_duration)
